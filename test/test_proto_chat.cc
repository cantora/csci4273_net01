...
BOOST_AUTO_TEST_CASE(send_msg) {
	...
	status = proto_chat::send_msg(s, *istrm, 0, sent);

	BOOST_REQUIRE_EQUAL(status, proto_chat::SND_DONE);	
	BOOST_REQUIRE_EQUAL(sent, msg_str.size()+6);
	close(s);		
}
...
BOOST_AUTO_TEST_CASE(recv_msg) {
	...
	if(pid == 0) {
		...
		status = proto_chat::send_msg(s, *istrm, msg_id, sent);
		...
		uint32_t recd_msg_id;

		status = proto_chat::RCV_CONT;
		while(status == proto_chat::RCV_CONT) {
			cout << '.';
			status = proto_chat::recv_msg(new_s, *ostrm, recd_msg_id, recd);
			sleep(0.1);
		}

		cout << endl << "got message from child: " << endl << from_child.str() << endl << "with msg_id: " << recd_msg_id << endl;
		...
		BOOST_REQUIRE_EQUAL(status, proto_chat::RCV_DONE);
		BOOST_REQUIRE_EQUAL(recd, m.size()+4);
		BOOST_REQUIRE_EQUAL(recd_msg_id, msg_id);
...
BOOST_AUTO_TEST_CASE(recv_long_msg) {
	...
	if(pid == 0) {
		...
		BOOST_REQUIRE_EQUAL(status,  proto_chat::RCV_DONE);
		BOOST_REQUIRE_EQUAL(recd, proto_chat::MAX_MSG_LEN+4);
		BOOST_REQUIRE_EQUAL(msg_id, recd_msg_id);
